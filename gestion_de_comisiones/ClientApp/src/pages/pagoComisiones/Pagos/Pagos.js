...
const Pagos = (props) => {
  ...
  async function verificarConfirmarSionPay(userN, cicloId) {
    let response = await Actions.verificarPagoSionPayXCiclo(
      userN,
      cicloId,
      dispatch
    );
  ...
  async function prosesarPagoSionPay(userN, usuarioId, cicloId) {
    let response = await Actions.pagarComisionSionPay(
      userN,
      usuarioId,
      cicloId,
      dispatch
  ...
  const handleTransferenciasEmpresas = async (user) => {
    ...
    if (idCiclo && idCiclo !== 0) {
      let response = await Actions.handleTransferenciasEmpresas(
        user,
        idCiclo,
        dispatch
      );
  ...
  useEffect(() => {}, [listaComisionesAPagar, listaComisionPaginacionNueva]);
  ...
  async function ApiVerificarConfirmarCierrePago(userNa, idUser, idCICLO) {
    if (idCICLO && idCICLO !== 0) {
      let response = await Actions.ConfirmarCierrePago(
        userNa,
        idUser,
        idCICLO,
        dispatch
                    ...
                    {validarPermiso(
                      ...
                      >
                        <Button variant="contained">
                          {" "}
                          <SaveIcon style={{ marginRight: "5px" }} /> CERRAR
                          PAGOS
                        </Button>
                      </Tooltip>
                    )}
