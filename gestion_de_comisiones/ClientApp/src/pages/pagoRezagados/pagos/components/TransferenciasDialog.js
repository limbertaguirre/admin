...
const TransferenciasDialog = ({
  ...
  const [
    openPagosTransferenciaDetalleDialog,
    setOpenPagosTransferenciaDetalleDialog,
  ] = useState(false);
  const [openModalCancel, setOpenModalCancel] = useState(false);
  const [isConfirmDialogType, setIsConfirmDialogType] = useState(false);
  const [selected, setSelected] = React.useState([]);
  const [transferenciaSeleccionData, setTransferenciaSeleccionData] =
    React.useState(null);
    ...
    if (cicloId && cicloId !== 0 && empresaId && empresaId != -1) {
      ...
      if (!response || !response.data) {
        dispatch(
          ActionMensaje.showMessage({
            message:
              "Hubo inconvenientes al recuperar la información. Intente nuevamente por favor.",
            variant: "error",
          })
        );
        return;
  ...
  const handleVerificarPagosTransferenciasTodos = async (user, empresaId) => {
    ...
    if (cicloId && cicloId !== 0 && empresaId && empresaId != -1) {
      ...
      if (
        response &&
        response.code == responseCodes.NO_EXISTEN_PENDIENTES_NI_RECHAZADOS
      ) {
        dispatch(
          ActionMensaje.showMessage({
            message: response.message,
        ...
        response &&
        response.code == responseCodes.EXISTEN_PENDIENTES
      ) {
        ...
        response &&
        response.code == responseCodes.EXISTEN_RECHAZADOS
      ) {
        setInputs(false);
        setDetalleTransferencia(data);
        setOpenPagosTransferenciaDetalleDialog(true);
  ...
  return (
    ...
    >
      ...
      <DialogActions>
        <Button
          disabled={!enabledConfirmarSeleccionInput}
          className={style.dialogConfirmButton}
          onClick={() => setOpenPagosTransferenciaDetalleDialog(true)}
        >
          Confimar todos
        </Button>
        <Button
          disabled={!enabledConfirmarSeleccionInput}
          className={style.dialogConfirmButton}
          onClick={() => handleObtenerPagosTransferencias(userName, empresaId)}
        >
          Confirmar seleccion
        </Button>
        <Button
          className={style.dialogConfirmButton}
          onClick={() => handleCloseTransferenciasDialog()}
        >
          Cerrar
        </Button>
      </DialogActions>
      {transferenciaSeleccionData && (
