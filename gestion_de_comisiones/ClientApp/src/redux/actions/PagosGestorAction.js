...
export async function ObtenerDetallePago(usuario, idDetalleComision, dispatch) {
    return new Promise((resolve) => {
        const data = {
            usuario,
            idDetalleComision: parseInt(idDetalleComision)
        };

        requestPost("gestionPagos/ObtenerDetalleComisionEmpresa", data, dispatch).then((response) => {
            resolve(response);
        });
    });
}
