...
BEGIN TRY
 ...
 ------------------------------------------------------------------ 
   ...
   INSERT INTO  @CLIENTES_CNX SELECT lcontacto_id,dtfechaadd, stelefonofijo, stelefonomovil,scorreoelectronico,dtfechanacimiento,sdireccion,scedulaidentidad, lpatrocinante_id, lnivel_id, snombrecompleto, scontrasena, CASE WHEN lcuentabanco IS NULL THEN 0 ELSE lcuentabanco END as 'lcuentabanco', lcodigobanco, cbaja, dtfechabaja,ltipobaja, smotivobaja, lnit    FROM OPENQUERY( [10.2.10.222], 'select * from administracioncontacto limit 200 ') order by lcontacto_id asc 
