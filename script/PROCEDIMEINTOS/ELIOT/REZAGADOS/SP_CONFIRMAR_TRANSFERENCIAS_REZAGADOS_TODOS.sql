    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            BEGIN TRANSACTION
                ...
                where   id_empresa = @EmpresaId and estado <> @EstadoConfirmado and
                        id_comision_detalle_empresa in (select i.id_comision_detalle_empresa from BDMultinivel.dbo.vwObtenerRezagadosPagos i
                                                where i.id_empresa = @EmpresaId and i.id_ciclo = @CicloId and i.id_comision = @ComisionId and i.id_tipo_pago = @TipoPagoTransferencia and i.id_estado_comision = @EstadoComisionId)
                                            
            COMMIT TRANSACTION
            select @CantidadActualizados = COUNT(*) from  BDMultinivel.dbo.vwObtenerRezagadosPagos i where i.id_empresa = @EmpresaId and i.id_ciclo = @CicloId and i.id_comision = @ComisionId and i.id_tipo_pago = @TipoPagoTransferencia and i.id_estado_comision_detalle_empresa <> @EstadoConfirmado and i.id_estado_comision = @EstadoComisionId
