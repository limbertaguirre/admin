...
GO
ALTER PROCEDURE [dbo].[SP3_PROCESAR_PAGO_INCENTIVO_SION_PAY_UPDATE]
     @id_Comision     int,
     @id_usuario int
AS
...
DECLARE @COMISIONES as table(id_comision int, idComisionDetalle  int, idFicha INT,nombre varchar(100), ci varchar(100), id_tipo_pago int, tipo_pago varchar(100), ciudad varchar(100), pais varchar(100), id_lista_formas_pago INT);
  ...
  DECLARE @DESCRIPCION_TIPO_INCENTIVO_PAGO VARCHAR(100);
	...
	SET @PARAM_ID_TIPO_COMISION_PAGO_COMISION=3; -- //GP_TIPO_COMISION incentivo
