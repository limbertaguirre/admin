  ...
  AS
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            COMMIT TRANSACTION                                     
            select @CantidadActualizados = COUNT(*) from  BDMultinivel.dbo.vwObtenerInfoExcelFormatoBanco i where i.id_empresa = @EmpresaId and i.id_ciclo = @CicloId and i.id_tipo_pago = @TipoPagoTransferencia and i.id_estado_comision_detalle_empresa <> @EstadoConfirmado
    ...
    BEGIN
        ...
        BEGIN CATCH   
            IF @@TRANCOUNT > 0
                BEGIN
                    SET @IMPBODY = concat ('SP_RECHAZAR_TRANSFERENCIAS_NO_SELECCIONADAS ', ' ');
                    SET @IMPSUBJECT = 'ALERTA PRODUCCION : No se pudo actualizar los estados de las comisiones en sion pay';
                    --EXECUTE msdb.dbo.sp_send_dbmail @profile_name   = 'NotificacionSQL',
                    --                                @recipients = 'desarrollo@gruposion.bo; UIT-SION@gruposion.bo',
                    --                                @body           = @IMPBODY,
    ...
    END
    GO
    CREATE PROC [dbo].[SP_REGISTRAR_TODAS_TRANSFERENCIAS_PAGOS_CONFIRMADAS](@cicloId int, @usuarioId int)
    AS
    BEGIN TRANSACTION
    BEGIN TRY
